...
export const whoisTransform: Transform = {
  ...
  async execute(entity: TransformInput): Promise<TransformResult> {
    ...
    try {
      ...
      console.error(`WHOIS lookup failed for ${entity.value}:`, error);
...
function parseWhoisResponse(raw: string): ParsedWhois {
  ...
  for (const line of lines) {
    ...
    // Organization
    if ((key.includes("org") && key.includes("name")) || key.includes("organization") || key === "org") {
    ...
    // Registrar
    if (key === "registrar" || key === "registrar name") {
      if (!result.registrar) result.registrar = value;
    }

    // Creation date
    if (key.includes("creation") || key.includes("created")) {
